"""
Benchmark script comparing GPyTorch and GPJax for heteroskedastic Gaussian Process regression
...
def generate_data(n_points=1000, t_df=3, t_loc=0.0, t_scale=2.1, seed=42):
    """
    ...
    x = stats.t.rvs(df=t_df, loc=t_loc, scale=t_scale, size=n_points)
    x = np.sort(x)  # Sort for nicer visualization
    
    # True function: power cosine
    y_true = np.power(np.abs(x), 0.7) * np.cos(2 * x)
...
def plot_results(x_train, y_train, x_test, y_true, 
    ...
    axes[0].fill_between(x_test, lower_gpytorch, upper_gpytorch, 
    ...
    axes[1].fill_between(x_test, lower_gpjax, upper_gpjax, 
...
def main():
    ...
    x_train, y_train, y_true_train, sigma_x = generate_data(
    ...
    print("-"*50)
    print(f"{'Speedup':<15} {train_time_gpytorch/train_time_gpjax:<15.2f}x {pred_time_gpytorch/pred_time_gpjax:<15.2f}x")
